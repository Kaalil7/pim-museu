<?php
	...
	//Executing the query
	// Verifique se a coluna codigo_acesso já existe na tabela member
	// Verifique se a coluna codigo_acesso já existe na tabela member
$queryCheckColumn = "PRAGMA table_info(member)";
$result = $conn->query($queryCheckColumn);
$columnExists = false;

foreach ($result as $column) {
    if ($column['name'] === 'codigo_acesso') {
        $columnExists = true;
        break;
    }
}

// Se a coluna codigo_acesso não existir, adicione-a
if (!$columnExists) {
    $queryAddColumn = "ALTER TABLE member ADD COLUMN codigo_acesso TEXT";
    $conn->exec($queryAddColumn);
}
?>